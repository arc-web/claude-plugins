...
import type { EntityInfo, EntityRole, EntitySource, GraphNode } from "./types.js";
...
function primaryEntity(tableName: string): string {
...
function detectRole(filePath: string, node: GraphNode): EntityRole {
  ...
  if (rel.includes("/api/") || /\/route\.(ts|js)x?$/.test(rel)) return "api";
  if (rel.includes("/hooks/") || /\/use[A-Z]/.test(filePath)) return "hook";
  if (rel.includes("/components/")) return "component";
  if (rel.includes("/types/") || rel.endsWith(".types.ts") || rel.endsWith(".d.ts")) return "type";
...
interface DetectedEntity extends EntityInfo {
  tables: string[]; // Original table names for compound matching
}

export function detectAndStoreEntities(
  store: GraphStore,
  repoRoot: string,
): { entityCount: number; memberCount: number } {
  ...
  for (const filePath of allFiles) {
    ...
    if (!rel.includes("migration") && !filePath.endsWith(".sql")) continue;
    ...
    try {
      ...
      for (const match of tableMatches) {
        ...
        if (primary.length < 3) continue; // Skip tiny names (id, etc.)
  ...
  for (const filePath of allFiles) {
    const rel = relative(repoRoot, filePath);
    if (!/\/(page|route)\.(tsx?|jsx?)$/.test(rel)) continue;

    const segments = rel.split("/").filter(
      (s) => !SKIP_ROUTE_SEGMENTS.has(s) && !s.startsWith("[") && !s.startsWith("("),
    );
    // Take the last meaningful segment before the filename
    const routeSegment = segments[segments.length - 2];
    if (!routeSegment || routeSegment.length < 3) continue;
  ...
  for (const filePath of allFiles) {
    ...
    if (!dirName || dirName.includes(".") || dirName.length < 3) continue;
  ...
  for (const filePath of allFiles) {
    const rel = relative(repoRoot, filePath);
    if (!rel.includes("/hooks/")) continue;

    const afterHooks = rel.split("/hooks/")[1];
    if (!afterHooks) continue;
