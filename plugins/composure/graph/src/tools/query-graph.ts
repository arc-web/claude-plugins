...
type QueryResult = { results: Record<string, unknown>[]; edges: Record<string, unknown>[] };

function edgesByTarget(store: GraphStore, qn: string, edgeKind: string): QueryResult {
  ...
  for (const e of store.getEdgesByTarget(qn)) {
    if (e.kind === edgeKind) {
      ...
      if (node) results.push(nodeToDict(node));
    }
  }
  return { results, edges };
}

function edgesBySource(store: GraphStore, qn: string, edgeKind: string): QueryResult {
  const results: Record<string, unknown>[] = [];
  const edges: Record<string, unknown>[] = [];
...
function callersOf(store: GraphStore, qn: string, nodeName: string | null): QueryResult {
...
function fileSummary(store: GraphStore, filePath: string): QueryResult {
  const results = store.getNodesByFile(filePath).map(nodeToDict);
  return { results, edges: [] };
}

function inheritorsOf(store: GraphStore, qn: string): QueryResult {
  const results: Record<string, unknown>[] = [];
  const edges: Record<string, unknown>[] = [];
  for (const e of store.getEdgesByTarget(qn)) {
...
export function queryGraph(params: {
  ...
  try {
    ...
    switch (pattern) {
      case "callers_of":    result = callersOf(store, qn, node?.name ?? null); break;
      case "callees_of":    result = edgesBySource(store, qn, "CALLS"); break;
      case "imports_of":    result = edgesBySource(store, qn, "IMPORTS_FROM"); break;
      case "importers_of":  result = edgesByTarget(store, qn, "IMPORTS_FROM"); break;
      case "children_of":   result = edgesBySource(store, qn, "CONTAINS"); break;
      case "tests_for":     result = edgesBySource(store, qn, "TESTED_BY"); break;
      case "inheritors_of": result = inheritorsOf(store, qn); break;
      case "file_summary":  result = fileSummary(store, node?.file_path ?? resolvedTarget); break;
