...
function classifyFileRole(filePath: string): string {
  ...
  if (/\/(app|pages)\/.*\/(page|layout|route)\.\w+$/.test(filePath)) return "route";
  if (/\/hooks\//.test(filePath)) return "hook";
  if (/\.(test|spec)\./.test(filePath)) return "test";
  if (/\/lib\//.test(filePath)) return "lib";
  if (/\.(config|rc)\.\w+$/.test(filePath)) return "config";
  if (/\/defaults\//.test(filePath)) return "defaults";
  if (/\/templates\//.test(filePath)) return "template";
  if (/\/steps\//.test(filePath)) return "skill-step";
  if (/\/references\//.test(filePath)) return "reference";
  return "source";
}
...
function parseRipgrepOutput(output: string, root: string, contextLines: number): Match[] {
  const matches: Match[] = [];
...
export function searchReferences(params: {
  ...
  const results = matches.map((m) => {
    ...
    let containingNode: string | null = null;
    ...
    if (store) {
      ...
      if (fileNode) {
        try {
          const rows = store.getDb()
            ...
            .all(fileNode.qualified_name) as { cnt: number }[];
      ...
      try {
        const entityRows = store.getDb()
          ...
          .all(fileNode?.qualified_name ?? "") as { entity_name: string }[];
