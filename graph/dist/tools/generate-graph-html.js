...
import { generateGraphHtml, CATEGORY_META, DEFAULT_CAT_ORDER, } from "../html-template.js";
const CATEGORY_RULES = [
    {
        key: "tests",
        match: (p, n) => /\.(test|spec)\.(ts|tsx|js|jsx)$/.test(n) ||
    ...
    {
        key: "config",
        match: (p, n) => /\.(config)\.(ts|js|mjs|cjs)$/.test(n) ||
...
function resolveImportTarget(specifier, sourceFile, fileSet) {
    ...
    for (const ext of extensions) {
        if (fileSet.has(join(stripped, `index${ext}`)))
            return join(stripped, `index${ext}`);
    }
    return null;
}
// ── Data extraction ───────────────────────────────────────────────
...
//# sourceMappingURL=generate-graph-html.js.map