...
import type { FindingCategory, FindingSeverity, ToolResult } from "../types.js";
...
function execSafe(cmd: string, args: string[], cwd: string): string | null {
  try {
...
function findPkgManager(root: string): string | null {
...
function toSeverity(raw: unknown): FindingSeverity {
...
type RecommendedAction = "split" | "split-on-next-touch" | "monitor" | "ignore";
...
function recommendForSize(
  lines: number,
  threshold: number,
  kind: "file" | "function",
): { recommendation: RecommendedAction; reason: string } {
...
function analyzeCohesion(
  store: GraphStore,
  db: ReturnType<GraphStore["getDb"]>,
  runId: string,
  repoRoot: string,
): number {
  let findingCount = 0;
  ...
  const filesWithManyFunctions = db
    .prepare(
      `SELECT f.file_path, f.qualified_name as file_qn, COUNT(n.id) as func_count
       FROM nodes f
    ...
    .all() as Array<{ file_path: string; file_qn: string; func_count: number }>;
  ...
  for (const file of filesWithManyFunctions) {
    ...
    const funcQns = db
      ...
      .all(file.file_path) as Array<{ qualified_name: string }>;
    ...
    const internalCalls = db
      ...
      .get(
        ...funcQns.map((r) => r.qualified_name),
        ...funcQns.map((r) => r.qualified_name),
      ) as { cnt: number };
    ...
    if (cohesionRatio < 0.3) {
      ...
      const fileSize = db
        ...
        .get(file.file_qn) as { lines: number } | undefined;
  ...
  const fanOutRows = db
    .prepare(
      `SELECT f.file_path, f.qualified_name as file_qn,
    ...
    .all() as Array<{ file_path: string; file_qn: string; call_files: number }>;
  ...
  for (const row of fanOutRows) {
    const funcCount = db
      ...
      .get(row.file_path) as { cnt: number };

    if (funcCount.cnt >= 5) {
      const fileSize = db
        .prepare("SELECT (line_end - line_start + 1) as lines FROM nodes WHERE qualified_name = ?")
        .get(row.file_qn) as { lines: number } | undefined;
      const lines = fileSize?.lines ?? 0;
  ...
  const singleCallerRows = db
    .prepare(
      `SELECT n.qualified_name, n.name, n.file_path,
  ...
  const singleImporterTypes = db
    ...
    name: string;
    file_path: string;
    importer_files: number;
    sole_importer: string;
  }>;

  // Group by file — only flag if a file has 3+ single-importer types
  const typesByFile = new Map<string, typeof singleImporterTypes>();
...
function analyzeCodeQuality(
  ...
  const fileRows = db
    .prepare(
      `SELECT qualified_name, file_path, (line_end - line_start + 1) as lines
       FROM nodes WHERE kind = 'File' AND (line_end - line_start + 1) > 400
       ORDER BY lines DESC`,
    )
    .all() as Array<{ qualified_name: string; file_path: string; lines: number }>;
  ...
  const funcRows = db
    .prepare(
      `SELECT qualified_name, name, file_path, (line_end - line_start + 1) as lines
...
function analyzeFileOrganization(
  ...
  try {
    ...
    const isNextJs = Object.values(frameworks).some(
      (fw) => (fw as Record<string, unknown>).frontend === "nextjs",
    );
    if (!isNextJs) return 0;
  } catch {
    return 0;
  }
  ...
  const rows = db
    .prepare(
      `SELECT qualified_name, file_path, name
       FROM nodes
       WHERE kind = 'File'
         AND name LIKE '%.tsx'
    ...
    .all() as Array<{ qualified_name: string; file_path: string; name: string }>;

  for (const f of rows) {
    if (NEXTJS_CONVENTION_FILES.has(f.name)) continue;
  ...
  if (pkgManager) {
    ...
    if (output) {
      try {
        ...
        const vulns = (data.vulnerabilities ?? data.advisories ?? {}) as Record<string, VulnEntry>;
...
export async function runAudit(params: {
  ...
  try {
    ...
    const availableCategories = new Set<FindingCategory>(["code-quality"]);
    ...
    const overallScore = scores.reduce(
      (sum, s) => sum + (s.raw_score as number) * (s.adjusted_weight as number),
      0,
    );
    ...
    const totalFindings = Object.values(findingCounts).reduce(
    ...
    const recCounts: Record<string, number> = { split: 0, "split-on-next-touch": 0, monitor: 0, ignore: 0 };
